...
class PgPaymentRepository {
  ...
  async save(payment) {
    ...
    const result = await this.pool.query(
      `INSERT INTO payments (id, order_id, status, transaction_id, amount, items)
       ...
       RETURNING *`,
      [payment.id, payment.orderId, payment.status, payment.transactionId, payment.amount, JSON.stringify(payment.items)]
    );
  ...
  async findByOrderId(orderId) {
    ...
    const row = result.rows[0];
    if (!row) return null;
    return new Payment(row.id, row.order_id, row.status, row.transaction_id, parseFloat(row.amount), row.items, row.created_at, row.updated_at);
  }
